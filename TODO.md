...
- [x] **Magnetic merge Stage 3.1** — chain-merge rigid-transform: sezení 13. `createFixedJoint(align=true)` rozcestník na 4 cesty (same-component / fresh-fresh / fresh+chain / chain+chain). Menší řetězec rigid-transformován **celý** (rotace o Δθ kolem guest_pixel + translace na targetGuestPos v dominantní ose host **local** frame). Internal joint anchory v body local frames zůstávají platné. ∑P preserved (V_unified=∑P/M), ω=0 (explicit angular momentum loss). Fresh-fresh symetrický snap zachován. Fixne uživatelův původní cluster bug ze S11/S12 + bonus fix `fresh+chain` s rotated chainem.
- [x] **Magnetic merge Stage 3.2** — composite rotation explicit handling: sezení 14. `Pixel.compositeTheta` field (sdílen members), `stepCompositesAlign(world, dt)` driveuje `θ_new = θ_old + ω·dt` + `setRotation` na všechny členy, `lockRotations` zcela odstraněno z align mode. Joint solver Rapieru volně iteruje rotation, my každý tick override z aggregate state. E14 sanity validace: identický s S3.1 (ω=0 → drift = 0); drobný ω drift 1.2×10⁻⁵ rad/s z f32 ulp (po odstranění lockRotations), geometrie rigidní (Y konzistentní s `sin(θ)·offset`).
